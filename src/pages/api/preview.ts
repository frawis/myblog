import { apiVersion, dataset, previewSecretId, projectId, useCdn } from '~/sanity/lib/sanity.api';
...
export default async function preview(req: NextApiRequest, res: NextApiResponse<string | void>) {
  ...
  if (!href) {
    return res.status(400).send('Unable to resolve preview URL based on the current document type and slug');
  }
