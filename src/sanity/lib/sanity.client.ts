...
export async function getHomePage({ token }: { token?: string | null }): Promise<HomePagePayload | undefined> {
  return await sanityClient(token)?.fetch(homePageQuery);
}

export async function getHomePageTitle({ token }: { token?: string | null }): Promise<string | undefined> {
...
export async function getSettings({ token }: { token?: string | null }): Promise<SettingsPayload | undefined> {
